...
T_sample       = 0.01;
N_samples      = 60;                % The total number of samples to run
N_add_samples  = 15;                % Number of additional samples after initial runs
N_total_sample = N_samples + N_add_samples;
T_final        = (N_total_sample)*T_sample;  % Total time for simulation (20s)

u_traj = -10 + 20*rand(N_total_sample,n_int);
u_time = [linspace(0,T_final,N_total_sample).', u_traj];
x0 = rand(n_states,1);
...
x_residual =  HL_x*alphas(:,1) - X_win(:,1);
disp('H_L(x^d)\alpha - x =');
disp(x_residual);
