function [H_L1,H_L2,f,Aeq,H1,F] = Get_MDL1_MHE_Param(P,R,rho,L,n_states,n_meas,A,B,C)
...
%                Minimize (1/2)||H_L2*z-f*xhat_old||_2^2 + (1/2)*||H_L1*z||_1^2
...
H_L2 = [(sqrt(rho)^L)*sqrt(P)*eye(n_states) zeros(n_states,n_meas*L)];
...
e_select = sqrt(R)*[zeros(n_meas*L,n_states) eye(n_meas*L)];
